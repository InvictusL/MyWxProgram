...
function wxSearchInput(e) {
  ...
  if (inputValue && inputValue.length > 0) {
    ...
    for (var i = 0; i < __tipKeys.length; i++) {
      var mindKey = __tipKeys[i]['bookname'];
      // 包含字符串
...
function search(inputValue) {
  if (inputValue && inputValue.length > 0 &&inputValue != '无搜索结果！') {
    // 添加历史记录
...
function wxSearchAddHisKey(inputValue) {
  ...
  if (value) {
    if (value.indexOf(inputValue) < 0) {
      value.unshift(inputValue);//向数组开头添加新元素
    }
    wx.setStorage({
      key: "wxSearchHisKeys",
      data: value,
    ...
    value = [];
    value.push(inputValue);//向数组末尾添加新元素
    wx.setStorage({
      key: "wxSearchHisKeys",
      data: value,
      success: function () {
